...
class DBHelper{
  ...
  Future<Database> openDB() async{
    ...
    String dbPath = join(appDir.path, "mainDBS.db");
  ...
  Future<bool> addNote({required NoteModel newNote}) async {
    ...
    return rowsEffected > 0; // Confirm rows were inserted
  }
  ...
  Future<bool> updateNote({required NoteModel updateNote}) async {
    ...
    int count = await db.update(
      ...
      //updateNote.toMap(),
      where: 'id = ?',
      whereArgs: [updateNote.id],
    );
    return count > 0; // Returns true if rows were updated
  }
